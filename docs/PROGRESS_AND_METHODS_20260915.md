...
資料核對日期：2026-09-15。用途：教授進度報告、畢業專題與履歷查閱。本文以「我做了什麼、為什麼做、得到什麼證據、還缺什麼」直接解釋每一個步驟。
...
| 優先序／問題 | 現在處理進度 | 接下來怎麼解決 | 何時才能說完成 |
|---|---|---|---|
| 1：新驗收RGB/BGR不一致 | 已定位程式差異，保留FAIL產物 | 先補帶顏色合成圖的輸入契約測試，再只重算固定191張開發驗收 | 新旧模型同輸入規則、完整分母與checkpoint身份一致，另產修訂報告 |
...
> 我建立了傷口影像分類、定位分割和護理覆核系統，並用內容指紋發現早期驗證存在重複資料，改為群組分割後保留較可信的開發結果。我沒有只追求最高mAP，而是實測裁切完整率、漏偵測、串接分類退步和外部來源失敗；也發現最新驗收有RGB/BGR輸入不一致，先將它列為評估修復問題。現在已完成多輪訓練與App回歸驗證，後續重點是修復驗收、补齊來源證據和取得新的合法未見資料。

履歷可用：「開發React／FastAPI傷口照護研究原型，整合YOLO分割與分類、MD5／SHA256／pHash資料稽核、群組交叉驗證、模型驗收門檻、RAG覆核追溯與安全預覽；透過配對錯誤分析辨識資料洩漏、裁切退步及色彩通道缺陷。」可連到本報告讓查閱者看到失敗案例、處理方法與真實進度，而非只展示單一最高分。
