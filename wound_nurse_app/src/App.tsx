...
function App() {
  ...
  const [guidanceDraft, setGuidanceDraft] = useState<GuidanceDraft>({ title: '', wound_classes: '', recommendation: '', rationale: '', precautions: '', evidence_level: 'clinical_consensus', source: 'head_nurse_review', source_emr_id: '', review_confirmed: false, deidentified_confirmed: false })
  ...
  const api = async (path: string, init: RequestInit = {}) => {
    ...
    if (activeToken.current !== token) throw new Error('已忽略前一工作階段的回應')
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setRagGuidance(await api(`/api/rag/guidance?limit=20${user?.role !== 'nurse' ? '&include_pending=true' : ''}`))
  ...
  const handleLogout = () => {
    ...
    setPatients([]); setRecords([]); setUsers([]); setAudit([]); setFeedback([]); setRagGuidance([]); setSystemHealth(null)
    setAnalysisFile(null); setImagePreview(null); setTreatment(''); setHumanReviewConfirmed(false); setLoading(false)
  ...
  const runAnalysis = async () => {
    if (!analysisFile || !selectedPatient) { showToast('請先選擇病人並上傳影像'); return }
    const revision = assessmentRevision.current
    setLoading(true)
    try { const form = new FormData(); form.append('file', analysisFile); const result = await api('/api/predict', { method: 'POST', body: form }) as Prediction; if (revision !== assessmentRevision.current) return; setPrediction(result); setHumanReviewConfirmed(false); if (result.low_confidence || result.fallback_used || result.advice_blocked) setReviewStatus('needs_review'); showToast(result.low_confidence || result.fallback_used || result.advice_blocked ? '此結果需要護理長或臨床人員進一步覆核' : '分析完成，請由護理人員確認後存檔') } catch (error) { showToast(error instanceof Error ? error.message : '影像分析失敗') } finally { if (revision === assessmentRevision.current) setLoading(false) }
  }
  ...
  const saveAssessment = async () => {
    ...
    if (loading) { showToast('請等待目前操作完成'); return }
    const revision = assessmentRevision.current
    setLoading(true)
    try {
      ...
      if (revision !== assessmentRevision.current) return
      assessmentRevision.current += 1; setAnalysisFile(null)
    ...
    } catch (error) { showToast(error instanceof Error ? error.message : '儲存失敗') } finally { setLoading(false) }
  }
  ...
  const saveGuidance = async () => {
    if (!guidanceDraft.review_confirmed || !guidanceDraft.deidentified_confirmed) { showToast('請確認內容已覆核且已去識別化'); return }
    ...
    try {
      ...
      setGuidanceDraft({ title: '', wound_classes: '', recommendation: '', rationale: '', precautions: '', evidence_level: 'clinical_consensus', source: 'head_nurse_review', source_emr_id: '', review_confirmed: false, deidentified_confirmed: false })
      showToast('護理長建議已寫入 RAG 知識庫'); setRagGuidance(await api(`/api/rag/guidance?limit=20${user?.role !== 'nurse' ? '&include_pending=true' : ''}`))
  ...
  const reactivateGuidance = async (guidanceId: number) => {
    try {
      await api(`/api/rag/guidance/${guidanceId}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_active: true, review_confirmed: true, deidentified_confirmed: true }) })
      showToast('已記錄覆核並啟用建議')
      setRagGuidance(await api('/api/rag/guidance?limit=20&include_pending=true'))
    } catch (error) { showToast(error instanceof Error ? error.message : '覆核失敗') }
  }

  const selectPatient = (patient: Patient | null) => {
    const revision = ++assessmentRevision.current
    setSelectedPatient(patient); setView('assessment'); setRecords([]); setLoading(false)
    setPrediction(null); setAnalysisFile(null); setImagePreview(null); setTreatment('')
    setHumanReviewConfirmed(false); setReviewStatus('needs_review')
    if (fileInput.current) fileInput.current.value = ''
    if (patient) void api(`/api/patients/${patient.id}/emr`).then(data => {
      if (revision === assessmentRevision.current) setRecords(data)
    }).catch(error => { if (revision === assessmentRevision.current) showToast(error.message) })
  }
  ...
  return <div className="shell">
    <aside className="sidebar">
      ...
      <div className={`unit-card ${systemHealth?.inference_ready ? '' : 'system-warning'}`}><span className="live-dot" /> {systemHealth?.inference_ready ? '影像分析模型已載入' : systemHealth?.status === 'ok' ? '系統可用 · 模型未就緒' : '正在確認系統狀態'}<div className="unit-caption">{systemHealth?.professor_preview ? '教授預覽版 · 去識別化資料' : `護理站 · ${shift}`}</div></div>
      <nav className="nav">
    ...
    <main className="main">
      ...
      {view === 'assessment' && prediction && <section className={`preview-safety ${prediction.low_confidence || prediction.fallback_used || prediction.advice_blocked ? 'attention' : ''}`}><div><strong>{prediction.low_confidence || prediction.fallback_used || prediction.advice_blocked ? '需要進一步覆核' : '人工覆核仍為必要步驟'}</strong><p>{prediction.low_confidence || prediction.fallback_used || prediction.advice_blocked ? '尚無可靠定位、分類信心不足或已使用原圖 fallback；請先人工確認，不要直接採用分類或處置建議。' : 'AI 輸出僅供決策支援，儲存前請確認傷口位置、分類與處置。'}</p>{prediction.model && <small>模型：{prediction.model.file} · SHA-256：{prediction.model.sha256?.slice(0, 12) ?? '未取得'} · 輸入 {prediction.model.input_size}px</small>}</div><label className="review-attestation"><input type="checkbox" checked={humanReviewConfirmed} onChange={e => setHumanReviewConfirmed(e.target.checked)} />我已親自覆核影像、AI 輸出與處置內容。</label></section>}
      {view === 'board' && <Board patients={filteredPatients} counts={counts} search={search} setSearch={setSearch} statusFilter={statusFilter} setStatusFilter={setStatusFilter} onSelect={selectPatient} onNew={() => setView('assessment')} loading={loading} />}
      {view === 'patients' && <Patients patients={filteredPatients} search={search} setSearch={setSearch} onNew={user.role === 'nurse' ? undefined : () => setView('admin')} onSelect={selectPatient} />}
      {view === 'assessment' && <Assessment patient={selectedPatient} patients={patients} setPatient={selectPatient} imagePreview={imagePreview} prediction={prediction} fileInput={fileInput} onImage={handleImage} runAnalysis={runAnalysis} loading={loading} treatment={treatment} setTreatment={setTreatment} status={status} setStatus={setStatus} reviewStatus={reviewStatus} setReviewStatus={setReviewStatus} humanReviewConfirmed={humanReviewConfirmed} setHumanReviewConfirmed={setHumanReviewConfirmed} save={saveAssessment} records={records.filter(record => record.patient_id === selectedPatient?.id)} />}
      {view === 'audit' && <Audit records={records} audit={audit} ragGuidance={ragGuidance} guidanceDraft={guidanceDraft} setGuidanceDraft={setGuidanceDraft} saveGuidance={saveGuidance} deactivateGuidance={deactivateGuidance} reactivateGuidance={reactivateGuidance} />}
...
function GuidanceConfirmations({ draft, setDraft }: { draft: GuidanceDraft; setDraft: (draft: GuidanceDraft) => void }) {
  return <div className="review-checklist">
    <label className="field-label">來源文件／院內流程名稱<input className="field" value={draft.source} onChange={e => setDraft({ ...draft, source: e.target.value })} /></label>
    <label className="review-confirmation"><input type="checkbox" checked={draft.review_confirmed} onChange={e => setDraft({ ...draft, review_confirmed: e.target.checked })} />我已覆核內容、依據與適用範圍。</label>
    <label className="review-confirmation"><input type="checkbox" checked={draft.deidentified_confirmed} onChange={e => setDraft({ ...draft, deidentified_confirmed: e.target.checked })} />我已移除姓名、床號、病歷號等識別資訊。</label>
    <p className="muted">這是人工核准的文字知識檢索，不會自動訓練 LLM 或影像模型。勾選確認不等同系統已自動完成去識別化。</p>
  </div>
}

function GuidanceReviewRow({ item, deactivate, activate }: { item: RAGGuidance; deactivate: (id: number) => void; activate: (id: number) => void }) {
  const [confirmed, setConfirmed] = useState(false)
  const ready = item.is_active && item.review_confirmed && item.deidentified_confirmed
  return <div className="rag-library-row"><div><strong>{item.title}</strong><small>{item.wound_classes || '一般照護'} · {item.evidence_level} · {item.source}</small><p>{item.recommendation}</p>
    {item.rationale && <p>依據：{item.rationale}</p>}{item.precautions && <p>注意：{item.precautions}</p>}
    <small>{item.reviewed_at ? `覆核時間：${item.reviewed_at}` : '尚無明確覆核紀錄'}</small>
    {!ready && <label className="review-confirmation"><input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} />我已重新覆核以上內容，並確認已去識別化。</label>}
  </div><div className="rag-row-actions"><span className="secure-tag">{ready ? '已覆核 · 可檢索' : '停用或待覆核 · 不檢索'}</span>
    {ready ? <button className="text-button danger-text" onClick={() => deactivate(item.id)}>停用</button> : <button className="primary-button compact" disabled={!confirmed} onClick={() => { activate(item.id); setConfirmed(false) }}>確認並啟用</button>}
  </div></div>
}
